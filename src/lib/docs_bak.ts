...
function loadDocs(): LoadedDocs {
  ...
  const sidebarSource = readJson(path.join(DATA_ROOT, "sidebar.json"));

  const resolveRoute = (url?: string, title?: string) => {
    if (!url) return undefined;
    if (pages.has(url) || virtualPages.has(url)) return url;

    if (title) {
      const matches = titleIndex.get(normalizeTitle(title)) ?? [];
      if (matches.length === 1) {
        aliasRoutes.set(url, matches[0]);
        return url;
      }
    }
  ...
  const buildSidebar = (
    ...
    return items.map((item) => {
      ...
      const rawChildren = [...(item.pages ?? []), ...(item.children ?? [])];
      const nodeUrl = item.url ?? (item.type === "group" ? inferGroupUrl(rawChildren) : undefined);
  ...
  const sidebar = buildSidebar(sidebarSource.sections ?? []);
...
export function getPageDescription(page: JsonRecord) {
  const value = page.description ?? page.subtitle ?? page.content?.introductoryText;
